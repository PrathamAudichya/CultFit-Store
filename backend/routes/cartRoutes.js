...
router.post('/', protect, async (req, res) => {
    ...
    try {
        ...
        console.log('[Cart] Item added for user', req.user.id, '| product:', productId);
        res.json({ message: 'Cart updated', cartCount: countResult[0].count });
    } catch (err) {
        console.error(err);
...
router.delete('/:productId', protect, async (req, res) => {
    try {
        ...
        const [countResult] = await pool.query('SELECT COUNT(*) as count FROM cart WHERE user_id = ?', [req.user.id]);
        console.log('[Cart] Item removed for user', req.user.id, '| product:', req.params.productId);
        res.json({ message: 'Item removed from cart', cartCount: countResult[0].count });
    } catch (err) {
        console.error(err);

...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initAnimations();
    setupSpinners();
    window.addEventListener('storage', updateCartUI); // Sync across tabs
});
            ...
            if (matches.length > 0) {
                suggBox.innerHTML = matches.map(p => {
                    ...
                    <div class="suggestion-item" onclick="window.location.href='product.html?id=${p.id}'">
                        <img src="${p.image_url}" style="width:30px; height:30px; border-radius:4px; object-fit:cover;" onerror="this.onerror=null; this.src='https://placehold.co/30x30/ebf0f7/a6b4c8?text=P';">
...
function saveCart() {
    ...
    updateCartUI(); // Real-time UI refresh
    syncCartToBackend();
}

function updateCartUI() {
    // 0. Refresh local cart variable from storage (crucial for cross-tab sync)
    ...
    // 2. Update Total Prices (if elements exist)
    const totalPrice = cart.reduce(
        (sum, item) => sum + (Number(item.discount_price) || Number(item.price) || 0) * (Number(item.quantity) || 1),
        0
    );
    const formattedTotal = window.formatPrice ? formatPrice(totalPrice) : `₹${totalPrice.toFixed(2)}`;
    
    // Generic selector for any "total" display across pages
    const totals = document.querySelectorAll('#hero-cart-total, #cartTotal, .cart-total-display');
    totals.forEach(el => el.textContent = formattedTotal);
}
    ...
    modal.innerHTML = `
        <div class="modal-content glass-card fade-in" style="transform: translateY(0);">
            ...
            <div style="display:flex; gap: 2rem; flex-wrap: wrap;">
                <img src="${product.image_url}" style="width:100%; max-width: 300px; height: 300px; object-fit: cover; border-radius: 12px; box-shadow: var(--shadow-soft);" loading="lazy" onerror="this.onerror=null; this.src='https://placehold.co/600x600/ebf0f7/a6b4c8?text=Product+Image';">
